 ...
 */
export function getPostseasonTeams(sheet: Sheet | null): { al: Team[]; nl: Team[] } {
  if (!sheet) {
    return { al: [], nl: [] };
  }
 ...
 */
export function getWorldSeriesChampions(sheet: Sheet | null): {
 ...
 */
export function getTeamFromPick(teamPick: TeamPick): Team | null {
...
export function getTeamsByConference(teamPicks: TeamPick[]): { al: Team[]; nl: Team[] } {
  ...
  return {
    al: teams.filter((t) => t.conference === Conference.AL).sort((a, b) => a.abbreviation.localeCompare(b.abbreviation)),
    nl: teams.filter((t) => t.conference === Conference.NL).sort((a, b) => a.abbreviation.localeCompare(b.abbreviation)),
  };
}
 ...
 */
export function getTeamsByConferenceSortedByName(teamPicks: TeamPick[]): { al: Team[]; nl: Team[] } {
  const teams = getTeamsFromPicks(teamPicks);
  return {
    al: teams.filter((t) => t.conference === Conference.AL).sort((a, b) => getFullTeamName(a).localeCompare(getFullTeamName(b))),
    nl: teams.filter((t) => t.conference === Conference.NL).sort((a, b) => getFullTeamName(a).localeCompare(getFullTeamName(b))),
  };
}
