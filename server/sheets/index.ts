...
export async function getSheetByGroupAndUser(groupId: string, userId: string): Promise<null | Sheet> {
  await dbConnect();
...
export async function getSheetByGroupAndUserPopulated(groupId: string, userId: string): Promise<null | Sheet> {
  await dbConnect();

  const sheet = await SheetModel.findOne({ group: groupId, user: userId }).populate('teamPicks.team');
...
export async function updateSheet(
  id: string,
  updates: Partial<Pick<Sheet, 'postseasonPicks' | 'submittedAt' | 'teamPicks' | 'worldSeriesPicks'>>,
): Promise<null | Sheet> {
  await dbConnect();
