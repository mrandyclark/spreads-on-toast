...
const generateChartData = (members: GroupMemberSummary[]) => {
...
const StandingsChart = ({ members }: StandingsChartProps) => {
					...
					<LineChart data={chartData} margin={{ bottom: 5, left: 0, right: 10, top: 5 }}>
						...
						{topMembers.map((member) => (
							<Line
								...
								name={member.name}
								stroke={
									chartConfig[member.id as keyof typeof chartConfig]?.color || 'hsl(var(--primary))'
								}
					...
					{topMembers.map((member) => (
						<div className="flex items-center gap-2 text-sm" key={member.id}>
							<div
								...
								style={{
									backgroundColor: chartConfig[member.id as keyof typeof chartConfig]?.color,
								}}
							/>
