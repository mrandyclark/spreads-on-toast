...
export function DatePicker({
  ...
  const initialDate = value ? parseYMD(value) : (maxDate ? parseYMD(maxDate) : { day: 1, month: new Date().getMonth() + 1, year: new Date().getFullYear() });
    ...
    <Popover onOpenChange={setOpen} open={open}>
      ...
      <PopoverContent align="end" className="w-auto p-3">
        ...
        <div className="grid grid-cols-7 gap-1 mb-1">
          {dayNames.map((day) => (
            <div
              className="h-8 w-8 text-center text-xs text-muted-foreground flex items-center justify-center"
              key={day}
            >
              {day}
            </div>
