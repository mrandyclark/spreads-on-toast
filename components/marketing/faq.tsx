    ...
    <section className="py-16 sm:py-24" id="faq">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <Accordion className="mt-12" collapsible type="single">
          {faqs.map((faq, index) => (
            <AccordionItem className="border-border/50 data-[state=open]:bg-muted/30" key={index} value={`item-${index}`}>
              <AccordionTrigger className="px-4 text-left text-base font-medium hover:text-primary hover:no-underline">
                {faq.question}
              </AccordionTrigger>
