...
import { getSignAction, getTeamsAction, updateSignConfigAction } from '../actions';
...
export function SignDetailClient({ signId }: { signId: string }) {
	...
	const toggleTeamInList = (
		...
		setList(
			list.includes(teamId) ? list.filter((id) => id !== teamId) : [...list, teamId],
		);
	};
							...
							<div className="space-y-4">
								<div className="flex items-center gap-3">
									<button
										...
										type="button">
										<span
											className={`bg-background absolute left-0.5 top-0.5 h-5 w-5 rounded-full shadow transition-transform ${
...
/**
 * Reusable team picker grouped by division
 */
function TeamPicker({
	onToggle,
	selectedTeamIds,
